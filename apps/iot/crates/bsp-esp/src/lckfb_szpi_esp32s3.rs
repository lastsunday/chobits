...
/// The LEDC timer the backlight channel is bound to. A LEDC channel holds a
/// borrow of its timer with the same lifetime, so the timer outlives the
/// channel from a `'static` cell instead of the stack.
...
pub enum BoardError {
    ...
    /// An I2C transaction (pin expander) failed.
    I2c(i2c_master::Error),
...
impl Board<'static> {
    pub fn new(peripherals: Peripherals) -> Result<Startup<'static>, BoardError> {
        ...
        pca9557.set_output(DVP_PWDN_BIT).map_err(BoardError::I2c)?;
        ...
        let light = DisplayLight::new(panel, Backlight::new(channel));
