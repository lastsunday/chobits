...
`apps/iot` 只保留硬件轴 feature，core 零 cfg：

```
iot-core     — 无 [features]
iot-bsp-esp  — esp32c6-devkitc-1
iot-app      — esp32c6（chip 别名）/ esp32c6-devkitc-1（default）
```

CI 固定 `--no-default-features --features <board>`，构建的即全功能固件。将来引入任何软 feature，都必须重新满足四判据 + 六规则，并补齐矩阵验证。