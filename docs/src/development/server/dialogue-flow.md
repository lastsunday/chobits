...
三种模式由 Session 根据 Hello 消息中的 `listen_mode` 字段选择，底层均使用同一套 VAD + Listener 实现。详见 [VAD & Listener](../debugging/vad-listener.md)。
...
详细的协议字段定义见 [WebSocket Protocol](./websocket-protocol.md)。
